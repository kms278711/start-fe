...
for(var i=page*3; i<page*3+3; i++){
    str += '<img title="'+todayPhoto[i].title+'" onclick="javascript:window.open(&#39;'+todayPhoto[i].url+'&#39;)" src="'+todayPhoto[i].img+'" id="'+todayPhoto[i].id+'">';
}
str += '<br><button type="button" id="pre" onclick="javascript:pre()"><</button>';
str += '1 / ' + Math.ceil(todayPhoto.length/3);
str += '<button type="button" id="next"  onclick="javascript:next()">></button>';
wrap.innerHTML = str;
...
function show() {
    ...
    for(var i=page*3; i<end; i++){
        str += '<img title="'+todayPhoto[i].title+'" onclick="javascript:window.open(&#39;'+todayPhoto[i].url+'&#39;)" src="'+todayPhoto[i].img+'" id="'+todayPhoto[i].id+'">';
    }
    str += '<br><button type="button" id="pre" onclick="javascript:pre()"><</button>';
    str += page+1+' / '+ Math.ceil(todayPhoto.length/3);;
    str += '<button type="button" id="next" onclick="javascript:next()">></button>';
    wrap.innerHTML = str;
}